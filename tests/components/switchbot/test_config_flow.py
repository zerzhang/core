...
async def test_user_oauth_login(
    ...
    caplog.set_level(
        logging.DEBUG, logger="homeassistant.components.switchbot.config_flow"
    )
    result = await _async_start_user_oauth(hass)
    oauth_flow_id = result["flow_id"]
    ...
    oauth_log_text = "\n".join(
        ...
        if record.name
        in {"homeassistant.components.switchbot.config_flow", "switchbot.oauth"}
    )
    assert "Starting SwitchBot OAuth flow; continuation=select_device" in oauth_log_text
    assert "Resolving SwitchBot OAuth callback" in oauth_log_text
    assert "SwitchBot cloud device metadata loaded successfully" in oauth_log_text
    assert f"flow={oauth_flow_id[-8:]}" in oauth_log_text
    assert "source=user" in oauth_log_text
    assert "callback_host=my.home-assistant.io" in oauth_log_text
    assert "refresh_token_present=True" in oauth_log_text
    assert "refresh_expires_in_present=True" in oauth_log_text
    assert "duration_ms=" in oauth_log_text
    assert "Creating SwitchBot device entry;" in oauth_log_text
    assert "device=****EEFF" in oauth_log_text
    assert "oauth_used=True oauth_token_stored=False" in oauth_log_text
    for sensitive_value in (
        ...
        assert sensitive_value not in oauth_log_text
...
async def test_encrypted_device_oauth_login(
    ...
    oauth_log_text = "\n".join(
        record.getMessage()
        for record in caplog.records
        if record.name == "homeassistant.components.switchbot.config_flow"
    )
    assert "Retrieving SwitchBot encryption key with OAuth" in oauth_log_text
    assert "SwitchBot OAuth encryption key retrieval succeeded" in oauth_log_text
    assert "device=****EEFF" in oauth_log_text
    assert "duration_ms=" in oauth_log_text
    assert "oauth_used=True oauth_token_stored=False" in oauth_log_text
    for sensitive_value in (
        OAUTH_ACCESS_TOKEN,
        "authorization-code",
        WOLOCK_SERVICE_INFO.address,
        key_details[CONF_ENCRYPTION_KEY],
    ):
        assert sensitive_value not in oauth_log_text
...
@pytest.mark.parametrize(
    ("status", "reason"),
    [
        pytest.param(400, "oauth_unauthorized", id="client-error"),
        pytest.param(429, "oauth_failed", id="rate-limited"),
